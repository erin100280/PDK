...
if(2) {//- pageBuilder
	if(2) {//- private
		...
		,	pbResetSonSync=function() {
				...
				if(sm.applets) {
					son.applets=Read(Join(dir, './son/son.applets.js'));
					doCore=2;
				};
	...
	,	PUBLIC: {
		...
		,	$DoInitScript: function(req, res, op, cb) {
	         ...
	         res.write(sFirst+'son.applets'+sAfterName+son.applets+sLast);
...
if(2) {//- app
	...
	,	PUBLIC: {
		...
		,	Run: function(port, cb) {
				...
				CreatePage=function() {
					this.pageBuilder=new pageBuilder(o, function(err, obj){
						page=obj;
						CreateRouter();
					});
				};
				...
				CreateRouter=function() {
					if(ort) {
						...
						if(rt)
							itms.PushL({ mode: 'get', regex: rt, handler: page });
					}
					else
						ort={ items: [
							{
