...
(function() {
	...
	if(2) {//-vars
	   ...
	   ;
		...
		if(cSide) {
			...
			O.modPath=z;
			O.execPath=(z=process.execPath);
			O.execDir=require('path').dirname(z);
	...
	if(2){//-Native.
	   ...
	   String.Sys().Implement({
	         $class: String
	      ,  PrimSys:{
	            	BeginsWith: function(val, matchCase) {
	            		var rv=false, z
	            		,	s=(matchCase)? this : this.LCase()
	            		,	sl=s.length
	            		,	v=(matchCase)? val : val.LCase()
	            		,	vl=v.length
	            		;
	            		
	            		if(s==v) rv=true;
	            		else if(sl>vl)
	            			if(s.substr(0, vl)==v) rv=true;

	            		return rv;
	            	}
	            ,	EndsWith: function(val, matchCase) {
	            		var rv=false, z
	            		,	s=(matchCase)? this : this.LCase()
	            		,	sl=s.length
	            		,	v=(matchCase)? val : val.LCase()
	            		...
	            		if(s==v) rv=true;
	            			...
	            			if(z=s.substr(sl-(0+vl), vl)==v) rv=true;
	...
	if(2){//-Setup system nodes.
	   if(!G.$$modes){
	      z=G.$$modes={};
	      z.os=z.$os=(cSide)? 'client' : process.platform;
	   };
	};
	...
	if(2) {//- Global
		...
		O.Global[((cSide)? 'Son' : 'Pops')]=function(doIt) {
	   	if(!$GLOBAL.$$_P_||doIt){O.Global(O); $GLOBAL.$$_P_=2; };
	   	return O;
		};
	};
	if(2) {//- IID
