	   ...
	   ,  Start: function(cb){
				...
				for(i=0, l=itms.length; i<l; i++) {
					...
					if(z.type=='staticServer') {
						if(!(uri=z.uri)) {//- error handler
							throw(new Error('uri must be set'));
							return;
						};
						z2=(k=z.options)? Object.Clone(k) : {};
						kk=uri;
						if(!kk.BeginsWith('/')) kk='/'+kk;
						//z.regex='^'+uri;
						z.regex='^'+kk;
						z.mode='get';
						if(!uri.EndsWith('/')) uri+='/';
...
var ss=X.staticServer=Class('pops.http:staticServer', {
	...
	,	INIT:function(op, cb){
			var o=this.SetOptions(op).OP, cnt;
			this.$nm=o.name;
	...
	,	PUBLIC: {
			...
			,	$srv: null
			,	$baseDir: 0
			,	$ServeFile: function(path, req, res) {
         		var rs
			...
			,	$SendIndex: function(path, req, res, mode) {
					if(this.OP.useIndexFiles)
						this.$SendIndex_File(path, req, res, mode);
					else
						this.$SendIndex_List(path, req, res, mode);
				}
			,	$SendIndex_File: function(path, req, res, mode) {
					var t=this, z;
					if(this.OP.useIndexFiles) {
						$fs_stat(z=$path_join(path, '/index.html'), function(err, v) {
							if(!err && v.isFile()) t.$ServeFile(z, res);
								...
								$fs_stat(z=$path_join(path, '/index.htm'), function(err, v) {
									if(!err && v.isFile()) t.$ServeFile(z, res);
									else t.$SendIndex_List(path, req, res, mode);
								});
						});
					}
					else t.$SendIndex_List(path, req, res, mode);
				}
			,	$SendIndex_List: function(path, req, res, mode) {
					var t=this, i, l, pth='', uri=this.uri||'', z;
					if(this.OP.rootAccess) {
						$fs_readDir(path, function(err, data) {
							if(err) res.end('ERROR: '+err);
							else
								var Fn, i=0, l=data.length, z, zz;
								if(!req.url.EndsWith('/')) {
									res.statusCode = 302;
									...
									Fn=function() {
										if(i<l) {
											z=zz=data[i];
											i++;
											$fs_stat($path_join(path, z), function(err, dat) {
												if(err) throw(new Error(err));
													...
													res.write('<a href="'+(uri+pth)+(z)+'">'+zz+'</a><br />');
												};
												Fn();
											});
										}
			...
			,	ProcessReq: function(req, res, Next) {
	            ...
	            $fs_stat(path, function(err, v) {
	            	if(err) {
		            	...
		            	if(v.isFile()) t.$ServeFile(path, req, res);
		            	else if(v.isDirectory()) t.$SendIndex(path, req, res);
							else {
								res.writeHead(200, {"Content-Type": "text/plain"});
		   ...
		   ,  Refresh:function(){
					...
					this.$OnRequest=o.OnRequest;
					this.$router=r;
		      	this.initAddress=o.initAddress;
		      }
		}
});
